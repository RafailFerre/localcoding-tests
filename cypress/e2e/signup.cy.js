...
describe('SIGN UP', () => {
    ...
    describe('NEGATIVE', () => {
        ...
        it('Sign up with invalid email', () => {
            ...
            SignUpPage.labelValidationEmail.should('be.visible').and('have.text', `'email' is not a valid email`)
            SignUpPage.buttonSubmit.should('be.disabled')
        })
        it('Sign up with invalid password', () => {
            ...
            //cy.url().should('eq', 'https://coding.pasv.us/user/register')
            cy.location('pathname').should('eq', '/user/register')
            SignUpPage.labelValidationPassword.should('be.visible').and('have.text', 'Password must be min: 5 symbols.')
            SignUpPage.buttonSubmit.should('be.disabled')
        })
    })
})
